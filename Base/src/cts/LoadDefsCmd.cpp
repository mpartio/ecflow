...
: force_(force), defs_filename_(defs_filename)
{
   ...
   if (base64_validate(defs_filename)) {
      // defs_filename might be the definitions as a string. This is the method
      // when loading definitions through the rest api

      const std::string decoded = base64_decode(defs_filename_);
      load_ok = defs->restore_from_string(decoded, errMsg, warningMsg);
      defs_filename_ = "";
   }
